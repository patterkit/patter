...
performances, recording, per-line localisation - and Patter is written to address
exactly that. Our thanks to Inkle Studios and to Joseph Humfrey.
:::
...
- **Production, not just the first draft.** Patter tracks how finished each line
...
- **Reviewing and feedback, built in.** Threaded comments, suggested rewrites, and notes that
...
- **Catching dead content before players do.** A
...
- **Editing the running game live.** Save in Patterpad and a
