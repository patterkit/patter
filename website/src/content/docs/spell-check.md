...
The **Spell-check** switch turns checking on and off, and it's on by default.

**Dictionary** is the language to check against. **English (US)** and **English (UK)** are built in, so
...
**Import…** adds your own language. Pick any **Hunspell** `.dic` file (its matching `.aff` comes with
...
**Project dictionary** is a list of words to **always accept in this project**, such as character names,
places, and invented terms. This list **travels with the project** through version control, so the whole
team checks against the same words. Add, edit, or delete them here, or use "Add to dictionary" on a
flagged word in the editor.
