---
title: Languages
description: Declare the languages a Patter project ships in, and find the translation loop on the production side.
sidebar:
  label: Languages
---
...
Everything that happens *after* setup is the running-the-project side, covered in
[Localisation](/production/localisation/). That means exporting for translators (JSON / Excel /
PO), importing the results back, the staleness tracking that survives rewrites, and the
Embedded-vs-IDs-only choice of how strings ship in the build.
