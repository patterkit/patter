...
// option prompts are excluded (proposal §4.1 - "covered when offered / eligible
// / taken?" is ambiguous), but the content reached THROUGH an option is tallied
// normally, so a never-taken branch still reads 0%. The harness owns a single
// seeded PRNG used for both the random choice-picks and each run's engine seed,
// so a `--seed` makes the whole coverage run bit-for-bit reproducible (proposal
// §5 - a Patter improvement over Storylets' unseeded picks).
// ---------------------------------------------------------------------------
...
export function runCoverage(loaded: LoadedProject, options: CoverageOptions = {}, hooks: CoverageHooks = {}): CoverageReport {
  ...
  // NOT part of it (proposal §4.1). The first-seen wins, so a duplicate id can't double-count.
