...
describe("project session: create -> open -> read -> save -> play", () => {
  ...
  it("a scene's first save creates its authoring shard, and validate does not call that 'not in project'", async () => {
    // Open a project that has no authoring folder yet (one generated by another tool, say), edit a line,
    // save. The edit trail creates authoring/<scene>.patterx through the save itself. The in-memory model
    // must learn of that file (it used to know only the authoring-field writes), and the not-in-project
    // check must judge by folder, not by what the model happens to hold (the Hamlet demo, 2026-09-03).
    const dir = mkdtempSync(join(tmpdir(), "pp-first-save-"));
    const opened = await project.createProject(dir, "First Save");
    project.openProject(dir);
    const sceneId = opened.scenes[0]!.id;
    expect(existsSync(join(dir, "authoring"))).toBe(false);
    expect(project.validate().problems.filter((p) => p.category === "not-in-project")).toEqual([]);
    const src = project.readScene(sceneId);
    const loc = src.locSource.replace(/"([^"\n]{3,})"/, '"$1 (edited)"'); // change one source string
    expect(loc).not.toBe(src.locSource);
