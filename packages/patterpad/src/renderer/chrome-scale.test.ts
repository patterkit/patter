...
describe("chrome scale: the surface's mixed sheet", () => {
  it("chrome rules are px", () => {
    for (const sel of ["#hintbar", ".hint kbd", ".action-menu", ".action-mi", ".slash-menu", ".slash-item", ".target-picker", ".tp-row", ".cue-ac", ".overline", ".note-icon", ".comment-bubble", ".status-pill"]) {
