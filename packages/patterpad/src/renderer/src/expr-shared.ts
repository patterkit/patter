...
export function patterFunctions(cat: CatalogueEntry[]): FunctionTemplateSpec[] {
  ...
  // option stays discoverable - matching the storylets clause menu, which leads with it.
  ...
  return [
    checkFlags,
    // seen / visits are Patter-specific (no storylets equivalent) - insert then pick the node via its pill.
